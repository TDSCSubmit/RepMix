...
from utils.stylegan2ada.torch_utils import misc                  

#   from torch_utils import training_stats
from utils.stylegan2ada.torch_utils import training_stats       

#   from torch_utils.ops import conv2d_gradfix
from utils.stylegan2ada.torch_utils.ops import conv2d_gradfix    

#   from torch_utils.ops import grid_sample_gradfix
from utils.stylegan2ada.torch_utils.ops import grid_sample_gradfix     
...
def training_loop(
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):   #   进入迭代
            if batch_idx % phase.interval != 0:
                continue

            # Initialize gradient accumulation.初始化梯度
            if phase.start_event is not None:
                phase.start_event.record(torch.cuda.current_stream(device))
            phase.opt.zero_grad(set_to_none=True)
            phase.module.requires_grad_(True)

            # Accumulate gradients over multiple rounds.    计算梯度
        ...
        if (network_snapshot_ticks is not None) and (done or cur_tick % network_snapshot_ticks == 0):
            ...
            snapshot_network_pkls.append(snapshot_pkl)
